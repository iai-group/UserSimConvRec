...
class MovieStateTracker():
    ...
    def __init__(self, args):
        ...
        super(MovieStateTracker, self).__init__()
        ...
        self.domain = domain
        if self.domain in ['CamRest', 'SlotFilling']:
            self.DState = \
            ...
            self.context_slots = [x for x in self.ontology.ontology['informable'].keys() if x not in self.ontology.ontology['system_requestable']]
            self.DContext = \
                Context(
                    {'params' : self.context_slots})
        else:
    ...
    def initialize(self, args=None):
        ...
        if self.domain == 'Movie':
            self.DContext.initialize({'params' : self.context_slots})
    ...
    def update_state(self, dacts):
        ...
        for dact in dacts:
            if dact.intent in ['inform', 'offer']:
                ...
                self.DContext.request_feedback = True
                for param in dact.params:
                    if param.slot == 'name':
                        self.DContext.feedback_slot = param.value
                        #self.DContext.feedback.update({param.value:None})

            elif dact.intent == 'feedback_given':
                for param in dact.params:
                    if param.slot == self.DContext.feedback_slot:
                        self.DContext.feedback.update({param.slot:param.value})
                        self.DContext.feedback_slot = ''
                        self.DContext.request_feedback = False
                        self.DContext.feedback_given = True
    ...
    def update_state_db(self, inpt=None, db_result=None, sys_req_slot_entropies=None,
                        sys_acts=None):
        """
                ...
                for dact in inpt:
                    if dact.intent == 'offer':
                        ...
                        for result in db_result:
                            for inf in self.context_slots:
                                if result[inf] not in self.DContext.params[inf]:
                                    found = True
                                else: found = False
                            if found == True:
                                self.DState.item_in_focus = result
                                break
